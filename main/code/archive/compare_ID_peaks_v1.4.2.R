...
##
## Version: 1.4.2
## New updates: use updated data to evaluate 24/25 season
## -----------------------------------------------------------------------------
...
## Load RSVnet data
rsvnet = read_csv("data/RSV/RSV-NET_data/rsvnet_012925.csv")
...
flu_data = read_csv("data/FluSURV/fluSurv_012925.csv",
                    na = "null",
                    skip = 2)
...
rsvnet_adults = rsvnet %>%
  filter(age_cat == "≥18 years (Adults)",
         race == "All",
         sex == "All",
         state != "RSV-NET")
...
rsvnet_adults = rsvnet_adults %>%
  select(state, season, week_end, rate) %>%
  rename(rsv = rate)
...
ili_flu_rsv = ili_flu_rsv %>% mutate(
  season = case_when(
    week_end >= as.Date(paste0(format(week_end, "%Y"), "-06-01")) &
      week_end < as.Date(paste0(format(
        week_end + months(12), "%Y"
      ), "-05-31")) ~ paste0(format(week_end, "%y"), "-", format(week_end + years(1), "%y")),
    week_end >= as.Date(paste0(format(
      week_end - months(6), "%Y"
    ), "-06-01")) &
...
for (geo in ili_flu_rsv %>% filter(season == "16-17") %>% distinct(state) %>% pull()) {
  ...
  cv = cv.glmnet(
    x = data.matrix(data[, c('flu_rescaled', 'rsv_rescaled')]),
  ...
  fit = penalized(
    ili_rescaled ~ flu_rescaled + rsv_rescaled ,
    positive = TRUE,
    unpenalized = ~ 0,
    data = data,
...
## Idea: what if use min-max scaling to rescale the signals?
minmax = function(x) {
...
ggsave(
  file = "figures/16_17_peak_timing_minmax.png",
  height = 6,
  width = 10,
  units = "in",
  bg = "white"
...
ggsave(
  file = "figures/17_18_peak_timing_minmax.png",
  height = 6,
  width = 10,
  units = "in",
  bg = "white"
...
for (geo in ili_flu_rsv %>% filter(season == "18-19") %>% distinct(state) %>% pull()) {
  ...
  print(paste0("Processing for ", geo, " is complete."))
...
ggsave(
  file = "figures/18_19_peak_timing_minmax.png",
  height = 6,
  width = 10,
  units = "in",
  bg = "white"
...
# RSV in 5/10
ggsave(
  file = "figures/19_20_peak_timing_minmax.png",
  height = 6,
  width = 10,
  units = "in",
  bg = "white"
...
ggsave(
  file = "figures/21_22_peak_timing_minmax.png",
  height = 6,
  width = 12,
  units = "in",
  bg = "white"
...
signals_with_betas_22_23 %>% filter(state == "California") %>% arrange(week_end)

ggsave(
  "figures/22_23_peak_timing_minmax.png",
  height = 6,
  width = 12,
  units = "in",
  bg = "white"
...
ggsave(
  "figures/23_24_peak_timing_minmax.png",
  height = 6,
  width = 12,
  units = "in",
  bg = "white"
...
# 23-24 season ----
signals_with_betas_24_25 = NULL
for (geo in ili_flu_rsv_covid %>% filter(season == "24-25") %>% distinct(state) %>% pull()) {
  # Filter the data to only include data from the selected state and season
  data = ili_flu_rsv_covid %>% filter(state == geo, season == "24-25") %>%
    mutate(
      ili_rescaled = minmax(ili),
      flu_rescaled = minmax(flu),
      rsv_rescaled = minmax(rsv),
  ...
  signals_with_betas_24_25 = bind_rows(signals_with_betas_24_25, signals_tmp)

}
signals_with_betas_24_25 %>%
  ggplot(aes(x = week_end)) +
  geom_line(aes(y = flu_times_coef, color = 'Flu'), linewidth = 0.65) +
  geom_line(aes(y = rsv_times_coef, color = 'RSV'), linewidth = 0.65) +
  geom_line(aes(y = covid_times_coef, color = "COVID-19"), linewidth = 0.65) +
...
ggsave(
  "figures/24_25_peak_timing_minmax.png",
  height = 6,
  width = 12,
  units = "in",
  bg = "white"
...
find_states_with_early_rsv_peak(signals_with_betas_21_22) #9/10
...
find_states_with_early_rsv_peak(signals_with_betas_24_25) #1/10
...
combined_signals_ = combined_signals %>%
  group_by(season) %>%
  mutate(start_date = min(week_end),
         end_date = max(week_end)) %>%
  ungroup()
...
ranked_signals = combined_signals_ %>%
  pivot_longer(cols = all_of(ends_with("coef")),
               names_to = "variable",
               values_to = "value") %>%
