...
## 2. Load data ----------------------------------------------------------------
## The goal for the figure is to show 3 consecutive years to show the time order
## of peaks. Therefore we will have 3 years of data with different sets of signals
## in each year:
##
## * 21/22 season: Flu + COVID
## * 22/23 season: Flu + COVID + RSV
## * 23/24 season: Flu + RSV

## New update for the paper:
...
# flu_hosp_data = read_csv("data/ground_truth/RSV/flu_hosps_imputed.csv", na = "X")
# Don"t use the imputed hospitalization data because it uses ILI as part of the
# imputation methods
...
rsv_ili = rsvnet_adults %>% left_join(ili_data_with_dates, by = c("state", "week_end"))

# Let's make a test panel for Utah that would be part of figure 1
rsv_ili %>%
  filter(state == "Utah") %>%
  ggplot(aes(x = week_end)) +
  geom_line(aes(y = rate, color = "RSV")) +
  geom_line(aes(y = unweighted_ili, color = "ILI")) +
  scale_color_manual(values = c("RSV" = "dodgerblue3", "ILI" = "gold2"))
...
rsvnet_adults = rsvnet_adults %>% select(state, week_end, rate) %>%
  rename(rsv = rate)

ili_data_with_dates = ili_data_with_dates %>% select(state, week_end, unweighted_ili) %>%
  rename(ili = unweighted_ili)

ili_flu_rsv = rsvnet_adults %>% left_join(ili_data_with_dates, by = c("state", "week_end")) %>%
  left_join(flu_data_adults, by = c("state", "week_end"))

# To do the regression properly, we should consider times when Flu Surveillance was not
# recorded (i.e., flu off-season) as 0
...
# First, get the list of all unique states in the rsvnet group
...
# Utah
ili_flu_rsv_ut = ili_flu_rsv %>% filter(state == "Utah")
range(ili_flu_rsv_ut$week_end) #2018-2024

# Tennessee
ili_flu_rsv_tn = ili_flu_rsv %>% filter(state == "Tennessee")
range(ili_flu_rsv_tn$week_end) #2016-2024

# Oregon
ili_flu_rsv_or = ili_flu_rsv %>% filter(state == "Oregon")
range(ili_flu_rsv_or$week_end) #2016-2024

# New Mexico
ili_flu_rsv_nm = ili_flu_rsv %>% filter(state == "New Mexico")
range(ili_flu_rsv_nm$week_end) #2017-2024

# Minnesota
ili_flu_rsv_mn = ili_flu_rsv %>% filter(state == "Minnesota")
range(ili_flu_rsv_mn$week_end) #2016-2024

# Michigan
ili_flu_rsv_mi = ili_flu_rsv %>% filter(state == "Michigan")
range(ili_flu_rsv_mi$week_end) #2018-2024

# Georgia
ili_flu_rsv_ga = ili_flu_rsv %>% filter(state == "Georgia")
range(ili_flu_rsv_ga$week_end) #2016-2024

# Connecticut
ili_flu_rsv_ct = ili_flu_rsv %>% filter(state == "Connecticut")
range(ili_flu_rsv_ct$week_end) #2018-2024

# Colorado
ili_flu_rsv_co = ili_flu_rsv %>% filter(state == "Colorado")
range(ili_flu_rsv_co$week_end) #2018-2024

# California
ili_flu_rsv_ca = ili_flu_rsv %>% filter(state == "California")
range(ili_flu_rsv_ca$week_end) #2016-2024
...
all_objects = ls()
statewise_datasets = all_objects[grepl("^ili_flu_rsv_", all_objects)]

for(dat in statewise_datasets){

  dat = dat %>%
    mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
           ili_rescaled = ili/max(ili, na.rm = TRUE),
           rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

}

ili_flu_rsv_ca = ili_flu_rsv_ca %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_co = ili_flu_rsv_co %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_ct = ili_flu_rsv_ct %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_ga = ili_flu_rsv_ga %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_mi =  ili_flu_rsv_mi %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_mn =  ili_flu_rsv_mn %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_nm =  ili_flu_rsv_nm %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_or =  ili_flu_rsv_or %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_tn =  ili_flu_rsv_tn %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

ili_flu_rsv_ut =  ili_flu_rsv_ut %>%
  mutate(flu_rescaled = flu/max(flu, na.rm = TRUE),
         ili_rescaled = ili/max(ili, na.rm = TRUE),
         rsv_rescaled = rsv/max(rsv, na.rm = TRUE))

# We want to replicate the penalized regression process that we used before but for each year
...
signals_with_betas = NULL
for(geo in rsvnet_states){

  # Filter the data to only include data from the selected state
  data = ili_flu_rsv %>% filter(state == geo) %>%
  ...
  signals_with_betas = bind_rows(signals_with_betas, signals_tmp)
}
