...
create_comparison_plot = function(flu_data, comparison_data, comparison_type, season, type = "onset", base_color = "darkslateblue") {
  ...
  p = ggplot(merged_data, aes(x = comparison_date, y = flu_date)) +
    ...
    annotate(
      "label",
      x = max_date - date_range * 0.05,  # Position near right edge
      y = min_date + date_range * 0.05,  # Position near bottom edge
      label = season,
      hjust = 1,
      vjust = 0,
      size = 3.5,
      ...
      label.padding = unit(0.35, "lines"),  # Padding around text
...
## Onsets -- Flu vs. COVID
onset_plot_flu_cov_23_24 = create_comparison_plot(flu_dat_23, cov_dat_23, comparison_type = "covid", season = "23-24", type = "onset", base_color = "darkslategrey")
...
result_23_24 = result_23_24 %>%
  ...
  mutate(
    ...
    closest_covid_onset = if(length(unlist(covid_onset_list)) > 0) {
      closest_date = unlist(covid_onset_list)[which.min(abs(difftime(flu_onset, unlist(covid_onset_list), units = "days")))]
    ...
    closest_covid_peak = if(length(unlist(covid_peak_list)) > 0) {
      closest_date = unlist(covid_peak_list)[which.min(abs(difftime(flu_peak, unlist(covid_peak_list), units = "days")))]
      as.Date(closest_date, origin = "1970-01-01")
    } else {
      NA_Date_
    },
...
plot_data = result_combined %>%
  ...
  pivot_longer(
    cols = c("Flu-RSV Peak", "Flu-COVID Peak", "Flu-RSV Onset", "Flu-COVID Onset"),
    names_to = "comparison",
...
plot_data = plot_data %>%
  mutate(
    ...
    comparison = factor(comparison,
                        levels = c("Flu-RSV Peak", "Flu-RSV Onset",
                                   "Flu-COVID Peak", "Flu-COVID Onset")),
...
violin_peaks = plot_data %>% filter(str_detect(comparison, "Peak")) %>%
  ...
  geom_violin(position = position_dodge(width = 0.8), width = 1, alpha = 0.5, trim = FALSE) +
  ...
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.15,
               alpha = 0.9, outlier.shape = NA, color = "black") +
  ...
  # Clear labeling
  labs(
    x = "",  # No label needed for categorical x-axis
...
violin_peaks_horizontal = plot_data %>% filter(str_detect(comparison, "Peak")) %>%
  ...
  # Clear labeling
  labs(
    y = "",  # No label needed for categorical y-axis
    x = "Weeks Difference\n(+ means flu occurred later)",
    fill = "Season"
  ) +
  # Clean theme with good spacing
...
scatter_rsv_2x2
# ggsave(file = "figures/manuscript_figures/fig3/flu_rsv_peaks_onsets.png", width = 14, height = 10, units = "in", bg = "white")

## COVID
scatter_covid_2x2 = (peak_plot_flu_cov_23_24 + onset_plot_flu_cov_23_24)/
  (peak_plot_flu_cov_24_25 + onset_plot_flu_cov_24_25)
scatter_covid_2x2
# ggsave(file = "figures/manuscript_figures/fig3/flu_cov_peaks_onsets.png", width = 14, height = 10, units = "in", bg = "white")
...
violin_labeled_sq = violin_labeled +
  theme(
    aspect.ratio = 1,
    plot.tag.position = c(0.05, 0.98),  # Moved further right from the default
    plot.tag = element_text(size = 14, face = "bold")
  )
...
combined_peak_plot
ggsave("figures/manuscript_figures/fig2/fig2_050525.png", height = 10, width = 8, units = "in", bg = "white")
...
combined_onset_plot
ggsave("figures/manuscript_figures/supplementary/supp_fig_10_050525.png", height = 10, width = 8, units = "in", bg = "white")
