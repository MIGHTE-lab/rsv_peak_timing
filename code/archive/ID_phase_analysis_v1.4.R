...
nssp_signals_22_23 %>%
  pivot_longer(cols = c(flu_times_coef, rsv_times_coef, covid_times_coef, ili_rescaled),
               names_to = "virus", values_to = "value") %>%
  mutate(virus = factor(virus, levels = c("ili_rescaled", "rsv_times_coef", "flu_times_coef", "covid_times_coef"),
                        labels = c("ILI", "RSV", "Flu", "COVID-19"))) %>%
  ggplot(aes(x = week_end, y = value, color = virus)) +
...
# ggsave(file = "figures/nssp_22_23_040725.png", width = 16, height = 10, units = "in", bg = "white")
...
# ggsave(file = "figures/nssp_23_24_040725.png", width = 16, height = 10, units = "in", bg = "white")
...
## For the 24-25 season, exclude VT and WY (no data supplied at various times)
nssp_signals_24_25 %>%
...
# ggsave(file = "figures/nssp_24_25_040725.png", width = 16, height = 10, units = "in", bg = "white")
...
## 3. Evaluating temporal order of peaks based on regression method ----
...
find_states_with_early_flu_peak(nssp_signals_24_25 %>% filter(state != "Wyoming" & state != "Vermont")) #9
# 20; 20/148 = 0.135

find_states_with_early_rsv_peak(nssp_signals_22_23) #45
find_states_with_early_rsv_peak(nssp_signals_23_24) #35
find_states_with_early_rsv_peak(nssp_signals_24_25 %>% filter(state != "Wyoming" & state != "Vermont")) #34
# 114; 114/148 = 0.770

find_states_with_concurrent_rsv_peak(nssp_signals_22_23) #3
find_states_with_concurrent_rsv_peak(nssp_signals_23_24) #6
find_states_with_concurrent_rsv_peak(nssp_signals_24_25 %>% filter(state != "Wyoming" & state != "Vermont")) #5
# 14; 14/148 = 0.095
...
state_peaks <- nssp_signals_22_23 %>%
  group_by(state) %>%
  summarize(
    max_rsv_date = week_end[which.max(rsv_rescaled)],
    max_flu_date = week_end[which.max(flu_rescaled)],
...
# Find the min/max dates for RSV peaks
rsv_peak_summary <- state_peaks %>%
  summarize(min_rsv = min(max_rsv_date),
            max_rsv = max(max_rsv_date))

# Find states with RSV peaks on the earliest date
states_with_earliest_rsv <- state_peaks %>%
  filter(max_rsv_date == rsv_peak_summary$min_rsv) %>%
  pull(state)

# Find states with RSV peaks on the latest date
states_with_latest_rsv <- state_peaks %>%
  filter(max_rsv_date == rsv_peak_summary$max_rsv) %>%
  pull(state)

# Display the results
earliest_rsv_peak <- rsv_peak_summary$min_rsv
latest_rsv_peak <- rsv_peak_summary$max_rsv

states_with_earliest_rsv
states_wiht_latest_rsv
...
# Figure out when the early peaks happened and in which states ----
## 22-23 season
nssp_signals_22_23 %>%
  group_by(state) %>%
  summarize(
...
## 24-25 season
nssp_signals_24_25 %>%
  filter(state != "Wyoming" & state != "Vermont") %>%
  group_by(state) %>%
  summarize(
    max_rsv_date = week_end[which.max(rsv_rescaled)],
    max_flu_date = week_end[which.max(flu_rescaled)],
...
write_csv(nssp_all_years, file = "data/nssp_all_years_040725.csv")
# 4. Phase Analysis (unused) ------
