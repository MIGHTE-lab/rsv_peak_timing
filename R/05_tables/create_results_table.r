...
create_results_excel = function(model_results,
  ...
  comprehensive_data = model_results %>%
    ...
    mutate(
      ...
      across(starts_with("Î²"), ~ round(.x, 3)),
  ...
  if (include_summary) {
    ...
    summary_stats = model_results %>%
      ...
      summarise(
        `N State-Seasons` = n(),
        `Mean Î² Intercept` = round(mean(beta_intercept, na.rm = TRUE), 3),
        `Mean Î² COVID` = round(mean(beta_covid, na.rm = TRUE), 3),
        `Mean Î² Flu` = round(mean(beta_flu, na.rm = TRUE), 3),
        `Mean Î² RSV` = round(mean(beta_rsv, na.rm = TRUE), 3),
