...
create_unified_comparison_table = function(results_ridge_no_int,
  ...
  if (version == "simplified") {
    display_names = c(
      ...
      "N Weeks", "Î² Intercept", "Î² COVID", "Î² Flu", "Î² RSV",
  ...
  cat("Dimensions:", nrow(unified_table), "rows Ã—", ncol(unified_table), "columns\n")
  cat("State-seasons included:", nrow(unified_table), "\n")

  if (version == "simplified") {
    cat("Columns per model: 5 (N, Î²_intercept, Î²_covid, Î²_flu, Î²_rsv)\n")
  } else {
